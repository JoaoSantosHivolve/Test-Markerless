const unityInstance = UnityLoader.instantiate("unityContainer", "Build/Test-Markerless.json");
let WebXR;
window.ARWT = {}
...
window.ARWT.onButtonClicked = () => {
    if(!xrSession){
        ...
        navigator.xr.requestSession('immersive-ar', xrInit).then(onSessionStarted, onRequestSessionError);
    }else{
        xrSession.end();
    }
}
